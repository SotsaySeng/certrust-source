...
export default {
   ...
   */
  async verifyCredential(credentialId: string, { fullView = false }: { fullView?: boolean } = {}) {
    try {
      // Public lookups are by credentialId only. Resolving sequential numeric
      // ids here let anyone walk every credential in the database.
   ...
   */
  async processCredentialResult(credential: CredentialWithRelations, { fullView = false }: { fullView?: boolean } = {}) {
    ...
    const serializedCredential = isCredentialPrivate(credential) && !fullView
      ? privateOpenBadgeStub(credential, strapi.config.get('server.url') || '')
      : await openBadgeService.serializeCredential(credential.id, { publicView: true });

    // An issuer suspended pending a trust & safety review (Terms s.7): its
    // credentials must not read as valid until the review is over.
    if (credential.issuer?.organization?.suspendedAt) {
      credentialsVerifiedTotal.inc({ result: 'invalid' });
      return {
        verified: false,
        checks: [
          { check: 'issuer_suspended', result: 'error', message: 'The issuer of this credential is suspended pending review.' }
        ],
        credential: serializedCredential,
        rawCredential: toPublicCredential(credential, { fullView })
      };
    }
