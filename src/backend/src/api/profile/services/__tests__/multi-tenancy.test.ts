...
describe('Multi-Tenancy Service', () => {
  ...
  describe('userOwnsProfile', () => {
    ...
    it('should return false for an unowned profile with someone else\'s email', async () => {
      const mockProfile = { id: 1, name: 'Recipient', email: 'jane@example.com', owner: null };

      (global.strapi.entityService.findOne as jest.Mock).mockResolvedValue(mockProfile);
      (global.strapi as any).db = { query: () => ({ findOne: jest.fn().mockResolvedValue({ email: 'intruder@example.com' }) }) };
    ...
    it('should return true for an unowned profile carrying the user\'s own email', async () => {
      const mockProfile = { id: 1, name: 'Recipient', email: 'Jane@Example.com', owner: null };

      (global.strapi.entityService.findOne as jest.Mock).mockResolvedValue(mockProfile);
      (global.strapi as any).db = { query: () => ({ findOne: jest.fn().mockResolvedValue({ email: 'jane@example.com' }) }) };
  ...
  describe('getUserOrganizationId', () => {
    it("returns the organization id from the user's profile", async () => {
      const mockProfiles = [{ id: 1, owner: { id: 123 }, organization: { id: 42 } }];
      (global.strapi.entityService.findMany as jest.Mock).mockResolvedValue(mockProfiles);

      const result = await service.getUserOrganizationId(123);
    ...
    it('returns null when the user has no profile with an organization', async () => {
      const mockProfiles = [{ id: 1, owner: { id: 123 }, organization: null }];
      (global.strapi.entityService.findMany as jest.Mock).mockResolvedValue(mockProfiles);
