 ...
 * @certrust/mcp server and how to configure it.
 *
...
export default defineEventHandler((event) => {
  ...
  return {
    ...
    name: 'Certrust',
    mcp_server: {
      package: '@certrust/mcp',
      install: 'npx -y @certrust/mcp',
      version: '0.1.0',
      transport: 'stdio',
      configuration: {
        required: [],
        optional: [
          {
            env: 'CERTRUST_API_URL',
            description: 'Base URL of your Certrust backend',
            default: api,
          },
          {
            env: 'CERTRUST_API_TOKEN',
    ...
    api: {
      base_url: `${api}/api`,
      // Swagger UI (the OpenAPI spec is embedded in the page; the backend
      // does not serve it as a separate JSON document)
